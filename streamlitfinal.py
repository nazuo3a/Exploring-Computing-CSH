...
def setup_korean_font():
    """
    1) 레포에 포함한 폰트(권장): ./fonts/NanumGothic.ttf 등
    2) 시스템 폰트 fallback
    """
...
DEFAULT_CHRONIC = "/mnt/data/노인의_성별_만성질병_종류별_유병률_의사진단_기준__및_현_치료율_20251214174945.xlsx"
DEFAULT_SUBJ    = "/mnt/data/노인의_주관적_건강상태_20251214230000.xlsx"
...
with st.sidebar:
    ...
    st.write(FONT_MSG)
    if not FONT_OK:
        st.caption("확실히 하려면 레포에 ./fonts/NanumGothic.ttf 추가 권장")
...
with tab2:
    ...
    if prev_col not in chronic_wide.columns or trt_col not in chronic_wide.columns:
        st.warning("예상한 컬럼명이 데이터에 없습니다. 현재 컬럼명을 확인하세요.")
